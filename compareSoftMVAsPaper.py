...
def lumiWeightedAverageVsPt(id, abseta):
	...
	f2022 = ROOT.TFile("softMuonMVA2022SplitEta/" + mainPath%("2022",id) + "NUM_%s_DEN_TrackerMuons_abseta_%s_vs_pt.root"%(id, abseta), "OPEN")
	f2022EE = ROOT.TFile("softMuonMVA2022_EESplitEta/" + mainPath%("2022_EE",id) + "NUM_%s_DEN_TrackerMuons_abseta_%s_vs_pt.root"%(id, abseta), "OPEN")
	f2023 = ROOT.TFile("softMuonMVA2023SplitEta/" + mainPath%("2023",id) + "NUM_%s_DEN_TrackerMuons_abseta_%s_vs_pt.root"%(id, abseta), "OPEN")
	f2023BPix = ROOT.TFile("softMuonMVA2023_BPixSplitEta/" + mainPath%("2023_BPix",id) + "NUM_%s_DEN_TrackerMuons_abseta_%s_vs_pt.root"%(id, abseta), "OPEN")
...
def main():
	...
	idNames = {"softID": "cut-based soft ID", "softMVARun2": "Run 2 soft muon MVA ID", "softMVARun3": "Purdue HGB", "softMVARun3XGBMedium": "Run 3 soft muon ID", "softMVARun3HGBMedium": "HGB Medium"}
	...
	for abseta in absetabins:
		...
		frame = plotPad.DrawFrame(3,0.4,10,1.1,";Probe muon p_{T} [GeV]; %s"%yLabel)
		...
		leg = ROOT.TLegend(0.42, 0.65, 0.89, 0.92,"","brNDC")
		leg.SetFillColor(10)
		leg.SetFillStyle(0)
		leg.SetLineColor(10)
		...
		frame = ratioPad.DrawFrame(3,0.85,10,1.15,";probe muon p_{T} [GeV]; Data / MC")
	...
	for pt in ptbins:
		...
		yLabel = 'Muon ID efficiency'
		frame = plotPad.DrawFrame(-2.4,0.4,2.4,1.1,";Probe muon #eta; %s"%yLabel)
		frame.GetYaxis().SetTitleFont(42)
		frame.GetYaxis().SetTitleSize(0.08)
		frame.GetYaxis().SetTitleOffset(0.9)
		frame.GetYaxis().SetLabelFont(42)
		...
		leg = ROOT.TLegend(0.47, 0.65, 0.94, 0.92,"","brNDC")
		leg.SetFillColor(10)
		leg.SetFillStyle(0)
		leg.SetLineColor(10)
		leg.SetTextFont(62)
		...
		ratioPad.SetTopMargin(0.03)
		ratioPad.SetBottomMargin(0.4)
		frame = ratioPad.DrawFrame(-2.4,0.75,2.4,1.15,";Probe muon #eta; Data / MC")
		...
		canv.Print("SoftMuonEff_Run3Paper_vsEta_pt%s.pdf"%pt)
		canv.Print("SoftMuonEff_Run3Paper_vsEta_pt%s.png"%pt)
